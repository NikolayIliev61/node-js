      ...
      (async () => {
        ...
        try {
          ...
          const result = await pool
            .request()
            .input("userEmail", sql.NVarChar(255), accountObj.userEmail).query(`
                            SELECT u.userId, u.userName, p.passwordValue
                            FROM Users u
                            JOIN LoginUserPassword p
          ...
          if (!result.recordset[0])
            throw {
              statusCode: 404,
              errorMessage: "User not found with provided credentials.",
            };
          if (result.recordset.length > 1)
            throw {
              statusCode: 500,
              errorMessage: "Multiple hits of unique data. Corrupt database.",
            };

          const bcrypt_result = await bcrypt.compare(
            accountObj.userPassword,
            result.recordset[0].passwordValue
          );
          if (!bcrypt_result)
            throw {
              statusCode: 404,
          ...
          if (error)
            throw {
              statusCode: 500,
              errorMessage: "Corrupt user account informaion in database.",
            };
          ...
          try {
            ...
            const result00 = await pool
              ...
              .query(`
                        ...
                        INSERT INTO LoginUserPassword([passwordValue], [userId])
            ...
            if (!result00.recordset[0])
              throw {
                statusCode: 500,
                errorMessage: "Something went wrong, login is not created.",
              };
