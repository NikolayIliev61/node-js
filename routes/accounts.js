...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    return res.send(JSON.stringify(account, token, accountObj));
  } catch (err) {
    console.log(err);
    ...
    if (!err.statusCode)
      return res
        .status(401)
        .send(
          JSON.stringify({ errorMessage: "Incorrect user email or password." })
        );
    ...
    return res
      .status(400)
      .send(
        JSON.stringify({ errorMessage: err.errorMessage.details[0].message })
      );
  }
});
  ...
  try {
    ...
    if (err.statusCode != 400)
      return res
        ...
        .send(JSON.stringify({ errorMessage: err }));
    return res
      .status(400)
      .send(
        JSON.stringify({ errorMessage: err.errorMessage.details[0].message })
